(() => {
  ...
  document.querySelectorAll("[data-view]").forEach((button) => button.addEventListener("click", () => { state.view = button.dataset.view; document.querySelectorAll("[data-view]").forEach((item) => item.classList.toggle("is-active", item.dataset.view === state.view)); renderCurrent(); }));
  ...
  $("#page-content").addEventListener("click", (event) => { const row = event.target.closest("[data-patient-id]"); const plan = event.target.closest("[data-open-plan]"); const go = event.target.closest("[data-go]"); const retry = event.target.closest("#retry-load"); if (row) showPatientDetail(state.patients.find((patient) => patient.id === Number(row.dataset.patientId))); if (plan) showPlanEditor(plan.dataset.openPlan); if (go) { state.view = go.dataset.go; renderCurrent(); } if (retry) renderCurrent(); });
